...
- Vite plugin path containment; HMR soft-fails on invalid JSON while serving the last valid document
- Init template version token replacement; `syncDocument` validation; codegen sanitization
- Grid command reliability and `lastError` feedback in the editor store
