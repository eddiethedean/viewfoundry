...
CLI scaffolding (`viewfoundry init`), real `@viewfoundry/vite` plugin with document HMR, and new `landing-page` / `dashboard-builder` examples. No document schema changes. See [Migration from 0.4 → 0.5](migration-0.4-0.5.md) and the [changelog](changelog.md).
